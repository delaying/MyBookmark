...
import { CHANGE_SITE_REQUEST, REMOVE_SITE_REQUEST } from "../reducers/post";
...
const Content = ({ site, subid }) => {
  const dispatch = useDispatch();
  const [option, setOption] = useState(false);
  const [siteState, onChangeSiteState, setSiteState] = useInput(false);
  const [link, onChangeLink, setLink] = useInput(site?.link || "");
  const [explain, onChangeExplain, setListExplain] = useInput(
    site?.explain || ""
  );
  ...
  const onChangeSite = useCallback(
    (e) => {
      e.preventDefault();
      console.log(link, explain, site.id, subid);
      dispatch({
    ...
    [site, subid, link, explain]
  );

  return (
    <>
      {subid == site.SubfolderId ? (
        <>
          <SubDiv>
            <TextDiv>
              <form id="site" onSubmit={onChangeSite}>
                {siteState ? (
                  <LinkInput value={link} onChange={onChangeLink} required />
                ) : (
                  <SiteA href={site?.link} target="_blank" rel="noreferrer">
                    {site?.link}
                  </SiteA>
                )}
                <Explain>
                  {siteState ? (
                    <ExplainInput value={explain} onChange={onChangeExplain} />
                  ) : (
                    <ContentTag siteData={site.explain} />
                  )}
                </Explain>
                {}
              </form>
              ...
              {option && !siteState ? (
                <>
                  <OptionDiv>
                    ...
                    <Option onClick={onRemoveSite}>삭제</Option>
                    <CloseIcon
                      onClick={() => {
                        setOption(!option);
                      }}
                ...
                >
                  <BiDotsHorizontalRounded color="gray" fontSize="1.5em" />
                </IconDiv>
              )}
              {siteState ? (
                <SubButton type="submit" form="site">
                  제출
                </SubButton>
              ) : (
                <></>
