    ...
    <>
      {data.UserId == me.id ? (
        <>
          <SubDiv>
            <TextDiv>
              <FolderInfo>[폴더:{data.Folder.content}]</FolderInfo>

              <form id="site" onSubmit={onChangeSite}>
                {siteState ? (
                  <LinkInput value={link} onChange={onChangeLink} required />
                ) : (
                  <SiteA href={data?.link} target="_blank" rel="noreferrer">
                    {data?.link}
                  </SiteA>
                )}
                <Explain>
                  {siteState ? (
                    ...
                    <ContentTag siteData={data.explain} />
                  )}
                </Explain>
                {}
              </form>
